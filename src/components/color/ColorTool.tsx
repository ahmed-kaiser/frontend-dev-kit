...
import { buildCSS, buildHTML, freshState } from "../../lib/colorModel";
...
export type Update = (fn: (draft: ColorState) => void) => void;
...
export default function ColorTool() {
  ...
  const commit = useCallback((producer: (prev: ColorState) => ColorState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as ColorState;
      fn(d);
      return d;
    });
  }, [commit]);

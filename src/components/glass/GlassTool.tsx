...
import { buildCSS, buildHTML, freshState } from "../../lib/glassModel";
import Topbar from "../Topbar";
import Toast from "../Toast";
import Controls from "./Controls";
import Preview from "./Preview";
import CodePanel from "../CodePanel";
import "./GlassTool.css";

export type Update = (fn: (draft: GlassState) => void) => void;
...
export default function GlassTool() {
  ...
  const commit = useCallback((producer: (prev: GlassState) => GlassState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as GlassState;
      fn(d);
      return d;
    });
  }, [commit]);
  ...
  return (
    <div className="main">
      <Topbar
        ...
        desc="Frosted-glass / backdrop-blur panels"

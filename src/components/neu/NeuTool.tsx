...
import { buildCSS, buildHTML, freshState } from "../../lib/neuModel";
...
export type Update = (fn: (draft: NeuState) => void) => void;
...
export default function NeuTool() {
  ...
  const commit = useCallback((producer: (prev: NeuState) => NeuState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as NeuState;
      fn(d);
      return d;
    });
  }, [commit]);

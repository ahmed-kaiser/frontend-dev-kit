...
import { buildCSS, buildHTML, freshState } from "../../lib/gradientModel";
...
export type Update = (fn: (draft: GradientState) => void) => void;

interface History {
  past: GradientState[];
  present: GradientState;
  future: GradientState[];
}
...
export default function GradientTool() {
  ...
  const commit = useCallback((producer: (prev: GradientState) => GradientState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as GradientState;
      fn(d);
      return d;
    });
  }, [commit]);
  ...
  return (
    <div className="main">
      <Topbar
        ...
        desc="Linear, radial & conic gradients"

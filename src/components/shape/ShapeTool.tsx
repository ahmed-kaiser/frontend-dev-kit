...
import { buildCSS, buildHTML, freshState } from "../../lib/shapeModel";
...
export type Update = (fn: (draft: ShapeState) => void) => void;
...
export default function ShapeTool() {
  ...
  const commit = useCallback((producer: (prev: ShapeState) => ShapeState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as ShapeState;
      fn(d);
      return d;
    });
  }, [commit]);
  ...
  return (
    <div className="main">
      <Topbar
        ...
        desc="Build shapes with clip-path"

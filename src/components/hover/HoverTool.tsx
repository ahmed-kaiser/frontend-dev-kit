...
import { buildCSS, buildHTML, freshState } from "../../lib/hoverModel";
...
export type Update = (fn: (draft: HoverState) => void) => void;
...
export default function HoverTool() {
  ...
  const commit = useCallback((producer: (prev: HoverState) => HoverState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as HoverState;
      fn(d);
      return d;
    });
  }, [commit]);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // let the browser handle native text undo while typing the label
      if (target && (target.tagName === "INPUT" || target.tagName === "TEXTAREA") &&
          (target as HTMLInputElement).type === "text") return;
  ...
  return (
    <div className="main">
      <Topbar
        title="Hover Effects"
        desc="Compose a :hover transition"

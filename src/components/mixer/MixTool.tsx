...
import { buildCSS, buildHTML, freshState } from "../../lib/mixModel";
...
export type Update = (fn: (draft: MixState) => void) => void;
...
export default function MixTool() {
  ...
  const commit = useCallback((producer: (prev: MixState) => MixState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as MixState;
      fn(d);
      return d;
    });
  }, [commit]);
  ...
  return (
    <div className="main">
      <Topbar
        title="Color Mixer"
        desc="Blend two colors across a stepped scale"

...
import { buildCSS, buildHTML, freshState } from "../../lib/radiusModel";
import Topbar from "../Topbar";
import Toast from "../Toast";
import Controls from "./Controls";
import Preview from "./Preview";
import CodePanel from "../CodePanel";
import "./RadiusTool.css";

export type Update = (fn: (draft: RadiusState) => void) => void;
...
export default function RadiusTool() {
  ...
  const commit = useCallback((producer: (prev: RadiusState) => RadiusState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as RadiusState;
      fn(d);
      return d;
    });
  }, [commit]);
  ...
  return (
    <div className="main">
      <Topbar
        ...
        desc="Compose per-corner and elliptical radii"

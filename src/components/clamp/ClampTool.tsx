...
import { buildCSS, buildHTML, freshState } from "../../lib/clampModel";
import Topbar from "../Topbar";
import Toast from "../Toast";
import Controls from "./Controls";
import Preview from "./Preview";
import CodePanel from "../CodePanel";
import "./ClampTool.css";

export type Update = (fn: (draft: ClampState) => void) => void;
...
export default function ClampTool() {
  ...
  const commit = useCallback((producer: (prev: ClampState) => ClampState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as ClampState;
      fn(d);
      return d;
    });
  }, [commit]);

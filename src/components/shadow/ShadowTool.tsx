...
import { buildCSS, buildHTML, freshState } from "../../lib/shadowModel";
...
export type Update = (fn: (draft: ShadowAppState) => void) => void;

interface History {
  past: ShadowAppState[];
  present: ShadowAppState;
  future: ShadowAppState[];
}
...
export default function ShadowTool() {
  ...
  const commit = useCallback((producer: (prev: ShadowAppState) => ShadowAppState, force = false) => {
    setHist((h) => {
      const now = Date.now();
  ...
  const update: Update = useCallback((fn) => {
    commit((prev) => {
      const d = structuredClone(prev) as ShadowAppState;
      fn(d);
      return d;
    });
  }, [commit]);
  ...
  return (
    <div className="main">
      <Topbar
        ...
        desc="Compose and stack box shadows"
